/**
 * Shared utility helpers used across the application.
 */

/**
 * Format a number with locale-aware thousands separators.
 * @param {number} n
 * @returns {string}
 */
export function formatNumber(n) {
  return new Intl.NumberFormat('es-MX').format(n);
}

/**
 * Format a UTC timestamp into a human-readable Spanish date string.
 * @param {string} isoString
 * @returns {string}
 */
export function formatDate(isoString) {
  return new Date(isoString).toLocaleDateString('es-MX', {
    day: '2-digit',
    month: 'short',
    year: 'numeric',
  });
}

/**
 * Normalize totem code values to a stable uppercase slug.
 * Examples: "Mesa 1" -> "MESA-1" ; "MESA-01" -> "MESA-01"
 *
 * NOTE: the physical database table remains named `tables` for compatibility,
 * but the product language in the UI and code is now "tótem"/"tótems".
 */
export function normalizeTotemCode(value) {
  const normalized = (value ?? '')
    .toString()
    .trim()
    .toUpperCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^A-Z0-9]+/g, '-');

  const cleaned = normalized.replace(/^-+|-+$/g, '').replace(/-+/g, '-');
  if (!cleaned) return '';

  return cleaned.length <= 24 ? cleaned : cleaned.slice(0, 24).replace(/-+$/, '');
}

export const normalizeTableCode = normalizeTotemCode;

/**
 * Validate general external business URLs.
 */
export function validateBusinessUrl(raw) {
  if (!raw) return false;
  const value = raw.toString().trim();
  if (!value) return false;

  try {
    const url = new URL(value);
    return ['http:', 'https:'].includes(url.protocol);
  } catch {
    return false;
  }
}

/**
 * Validate a WhatsApp value from either a phone number or a URL.
 */
export function validateWhatsappUrl(raw) {
  if (!raw) return false;
  const value = raw.toString().trim();
  if (!value) return false;

  if (value.startsWith('http://') || value.startsWith('https://')) {
    try {
      const url = new URL(value);
      const host = url.hostname.toLowerCase();
      const hasWhatsappHost = host.includes('wa.me') || host.includes('whatsapp.com') || host.includes('api.whatsapp.com');
      const digits = (url.searchParams.get('phone') ?? '').replace(/\D/g, '');
      return hasWhatsappHost && (digits.length >= 8 || url.pathname.length > 1);
    } catch {
      return false;
    }
  }

  const digits = value.replace(/\D/g, '');
  return digits.length >= 8 && digits.length <= 15;
}

/**
 * Validate a phone number or tel: URL.
 */
export function validatePhoneNumber(raw) {
  if (!raw) return false;
  const value = raw.toString().trim();
  if (!value) return false;

  if (value.toLowerCase().startsWith('tel:')) {
    return value.replace(/^tel:/i, '').replace(/\D/g, '').length >= 8;
  }

  const digits = value.replace(/\D/g, '');
  return digits.length >= 8 && digits.length <= 15;
}

/**
 * Map an action_type key to its Spanish display label.
 * @param {'google_review'|'menu'|'whatsapp'|'instagram'|'facebook'|'tiktok'|'website'|'phone'} type
 * @returns {string}
 */
export function actionLabel(type) {
  const labels = {
    google_review: 'Reseña en Google',
    menu: 'Menú Digital',
    whatsapp: 'WhatsApp',
    instagram: 'Instagram',
    facebook: 'Facebook',
    tiktok: 'TikTok',
    website: 'Sitio Web',
    phone: 'Llamada',
  };
  return labels[type] ?? type;
}

/**
 * Resolve the public base URL for the application.
 * Prefer the environment variable, then the current request host, then browser origin.
 * @param {Headers | { get: Function } | null} headers
 * @returns {string}
 */
export function getAppBaseUrl(headers) {
  const envUrl = process.env.NEXT_PUBLIC_APP_URL;
  if (envUrl && envUrl.trim()) {
    return envUrl.replace(/\/$/, '');
  }

  if (typeof window !== 'undefined') {
    return window.location.origin.replace(/\/$/, '');
  }

  if (headers && typeof headers.get === 'function') {
    const forwardedProto = headers.get('x-forwarded-proto') || 'http';
    const forwardedHost = headers.get('x-forwarded-host') || headers.get('host') || 'localhost:3000';
    return `${forwardedProto}://${forwardedHost}`.replace(/\/$/, '');
  }

  return 'http://localhost:3000';
}

/**
 * Build a WhatsApp click-to-chat URL, normalizing phone numbers.
 * Accepts formats like: +52 55 1234 5678, 5215512345678, etc.
 * @param {string} raw  Raw WhatsApp URL or phone number stored in DB
 * @returns {string}
 */
export function resolveWhatsAppUrl(raw) {
  if (!raw) return '#';
  // Already a full URL
  if (raw.startsWith('http')) return raw;
  // Strip non-digits and build wa.me link
  const digits = raw.replace(/\D/g, '');
  return `https://wa.me/${digits}`;
}
